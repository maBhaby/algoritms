...
// const list = new LinkedList(1).append(2).append(3).append(4);

// console.log('list.length', list.calcLength());
...
// function myReduce<T extends readonly any[], K>(
//   arr: T,
//   fn: (acc: K, cur: T[number], i: number, arr: T) => K,
//   acc: K
// ) {
//   let resVal = acc;

//   for (let i = arr.length - 1; i >= 0; i--) {
//     resVal = fn(resVal, arr[i], i, arr);
//   }
...
// /**
//  *  Реализовать бинарное дерево без null
//  */
...
// class EmptyWayTreeNode extends ValueTreeNode {
//   nodeType = 'empty';
// }

// class OneWayTreeNode extends ValueTreeNode {
//   nodeType = 'single';
//   child: ValueTreeNode;
//   type: 'left' | 'right';
//   constructor(value: any, child: ValueTreeNode, type: 'left' | 'right') {
//     super(value);
//     this.child = child;
//     this.type = type;
//   }
...
// class DoubleWayTreeNode extends ValueTreeNode {
//   nodeType = 'double';
//   leftChild: ValueTreeNode;
//   rightChild: ValueTreeNode;
//   constructor(value: any, leftChild: ValueTreeNode, rightChild: ValueTreeNode) {
//     super(value);
...
// const tree = new DoubleWayTreeNode(
//   1,
//   new OneWayTreeNode(2, new EmptyWayTreeNode(3), 'left'),
//   new OneWayTreeNode(4, new EmptyWayTreeNode(5), 'right')
// );
...
//   | {
//       nodeType: 'double';
//       node: DoubleWayTreeNode;
//     };

// const calTreeLength = (tree: ValueTreeNode): number => {
//   let length = 0;

//   const travesTree = (tree: ValueTreeNode) => {
//     length++;
//     console.log('tree.value', tree.value);

//     if (tree instanceof OneWayTreeNode) {
//       travesTree(tree.child);
//     } else if (tree instanceof DoubleWayTreeNode) {
//       travesTree(tree.leftChild);
//       travesTree(tree.rightChild);
//     }
//   };
...
// console.log('length tree', calTreeLength(tree));
...
//     node.children.reduceRight((acc, node_) => {
//       acc.push(node_);
//       return acc;
//     }, stack);
//   }
...
// console.log(dfs.prototype);

// console.log(dfs(tree1));

// function calcReapedChar(str: string) {
...
//     if (currentChar === arrStr[i + 1]) {
//       reapedCount++;
//     } else {
//       if (reapedCount) {
//         result += reapedCount + currentChar;
//       }
...
// console.log(calcReapedChar('aaabca'));
...
function createService(id: number) {
  ...
  async function task() {
    ...
    await new Promise((res) => setTimeout(res, delay));
    console.log(
      `Апи ${id} завершён (время: ${delay.toFixed(0)}ms) приоритет ${priority}`
    );
    return `result_${id}`;
  }
  return {
...
function createServiceErr(id: number) {
  const priority = Math.floor(Math.random() * 3) + 1;
  async function task() {
    const delay = Math.random() * 2000 + 500;
    await new Promise((res, rej) =>
      setTimeout(() => rej(new Error("IDI NAX")), delay)
    );
    console.log(
      `Апи ${id} завершён С ОШИБКОЙ (время: ${delay.toFixed(
        0
      )}ms) приоритет ${priority}`
    );
    return `result_${id}`;
  }
  return {
...
type ReturnData = ReturnType<typeof createService>;
...
async function execute(
  ...
  while (tasks.length > completeJobs.current) {
    const { task, id } = tasks[completeJobs.current];
    try {
      ++completeJobs.current;

      const res = await task();
      result.push({ result: res, id });
    } catch (error) {
      result.push({ error: error.message, id });
      throw error;
    }
  }
}
...
async function ping(len: number, n: number) {
  ...
  const tasks = Array.from({ length: len }, (_, i) =>
    i === 0 ? createServiceErr(i + 1) : createService(i + 1)
  ...
  await Promise.allSettled(workers.map((worker) => worker()));

  return result.sort((a, b) => a.id - b.id);
}

// ping(6, 3, ).then(console.log);

function binarySearch(sortedArray: number[], value: number) {
  let copy = [...sortedArray];

  while (copy.length !== 1) {
    let a = Math.floor(sortedArray.length / 2);
    const temp = copy[a];

    if (temp > value) {
      copy = copy.slice(a + 1, copy.length);
    } else if (temp < value) {
      copy = copy.slice(0, a - 1);
    } else if (value === temp) {
      copy = [value];
    }
  }
...
function fn(goods: number[], buyerNeeds: number[]) {
  const sortStuff = goods.toSorted((a, b) => a - b);

  buyerNeeds.map((needed) => {
    const finded = binarySearch(sortStuff, needed);
  });
}
